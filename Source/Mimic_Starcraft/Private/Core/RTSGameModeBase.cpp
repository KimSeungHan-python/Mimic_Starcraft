...
void ARTSGameModeBase::InitializePlayerState(APlayerController* NewPlayer, ARTSStartCamp* Camp)
{
	...
	FRTSLobbyPlayerInfo LobbyPlayerInfo;
	const bool bHasLobbyPlayerInfo = GetLobbyPlayerInfoForPlayer(NewPlayer, LobbyPlayerInfo);

	const int32 TeamNumber = bHasLobbyPlayerInfo
		? LobbyPlayerInfo.TeamNumber
		: PS->GetPlayerId();
	const FLinearColor TeamColor = bHasLobbyPlayerInfo
		? LobbyPlayerInfo.PlayerColor
		: GetColorForPlayer(NewPlayer, TeamNumber);
	const ERTSRace Race = bHasLobbyPlayerInfo
		? LobbyPlayerInfo.Race
		: GetRaceForPlayer(NewPlayer);

	PS->SetTeamInfo(TeamNumber, TeamColor, Race, Camp->CampIndex);
	PS->SetLobbySelection(Race, TeamColor);
	PS->SetRoomHost(bHasLobbyPlayerInfo && LobbyPlayerInfo.bIsRoomHost);
...
bool ARTSGameModeBase::GetLobbyPlayerInfoForPlayer(
	APlayerController* NewPlayer,
	FRTSLobbyPlayerInfo& OutPlayerInfo
) const
{
	const URTSGameInstance* GI = GetGameInstance<URTSGameInstance>();
	if (!GI)
	...
	if (NewPlayer && NewPlayer->IsLocalController() && GI->GetLocalLobbyPlayerInfo(OutPlayerInfo))
	{
		return true;
	}

	if (!GameState || !NewPlayer || !NewPlayer->PlayerState)
	{
		return false;
	}

	const int32 PlayerIndex = GameState->PlayerArray.IndexOfByKey(NewPlayer->PlayerState);
	return GI->GetLobbyPlayerInfoByIndex(PlayerIndex, OutPlayerInfo);
}
